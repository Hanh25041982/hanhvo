...
def main():
    st.title("ðŸ“„ PDF Gemini Converter")
    ...
    if uploaded_file is not None:
        ...
        st.info(f"ðŸ“Š File type: {file_type} | Size: {file_size/1024:.1f} KB")
        
        if st.button("ðŸ”„ Convert to Text", type="primary"):
            try:
                converter = GeminiConverter(api_key)
                
                prompt = """
                HÃ£y nháº­n diá»‡n vÃ  gÃµ láº¡i [CHÃNH XÃC] ná»™i dung trong file thÃ nh vÄƒn báº£n, táº¥t cáº£ cÃ´ng thá»©c ToÃ¡n Ä‘Æ°á»£c bá»c trong dáº¥u $
                [TUYá»†T Äá»I] khÃ´ng thÃªm ná»™i dung khÃ¡c ngoÃ i ná»™i dung trong file, [CHá»ˆ ÄÆ¯á»¢C PHÃ‰P] gÃµ láº¡i ná»™i dung thÃ nh vÄƒn báº£n.
                """
                
                with st.spinner("ðŸ”„ Converting file to text..."):
                    if file_type == "application/pdf":
                        try:
                            chunk_files, total_pages = split_pdf(uploaded_file)
                            st.info(f"ðŸ“„ PDF has {total_pages} pages, split into {len(chunk_files)} parts")
                            ...
                            for i, chunk_file in enumerate(chunk_files):
                                if os.path.exists(chunk_file):
                                    ...
                                    st.write(f"  â€¢ Part {i+1}: {os.path.basename(chunk_file)} ({size/1024:.1f} KB)")
                                else:
                                    st.write(f"  â€¢ Part {i+1}: âŒ File not found")
                            ...
                            for i, chunk_file in enumerate(chunk_files):
                                try:
                                    ...
                                    if not os.path.exists(chunk_file):
                                        st.error(f"âŒ Chunk file {i+1} not found: {chunk_file}")
                                        continue
                                    
                                    file_size = os.path.getsize(chunk_file)
                                    if file_size == 0:
                                        st.error(f"âŒ Chunk file {i+1} is empty")
                                        continue
                                    
                                    with st.expander(f"ðŸ“„ Processing Part {i+1}", expanded=False):
                                    ...
                                    if text and text.strip():
                                        ...
                                        st.success(f"âœ… Part {i+1} processed successfully ({len(text)} characters)")
                                    else:
                                        st.warning(f"âš ï¸ Part {i+1} returned empty text")
                                    
                                    progress_bar.progress((i + 1) / len(chunk_files))
                                    
                                except Exception as e:
                                    st.error(f"âŒ Error processing part {i+1}: {str(e)}")
                                    ...
                                    try:
                                        ...
                                        st.warning(f"âš ï¸ Could not clean up file {chunk_file}: {cleanup_error}")
                            
                            if all_text:
                                final_text = "\n\n--- Page Break ---\n\n".join(all_text)
                                st.session_state.converted_text = final_text
                                status_text.text("ðŸŽ‰ Conversion completed!")
                            else:
                            ...
                            st.error(f"âŒ PDF processing failed: {str(pdf_error)}")
                ...
                st.error(f"âŒ Conversion failed: {str(e)}")
    ...
    if 'converted_text' in st.session_state and st.session_state.converted_text:
        ...
        with col1:
            if st.button("ðŸ“‹ Copy Text"):
                ...
                st.info("ðŸ’¡ Select all text above and copy manually (Ctrl+A, Ctrl+C)")
                ...
                if result.returncode == 0:
                    if st.button("ðŸ’¾ Generate Word Document"):
                        try:
                            ...
                            st.error(f"âŒ Word conversion failed: {str(e)}")
                            st.info("ðŸ’¡ You can copy the text and paste it into a Word document manually")
                else:
                    st.warning("âš ï¸ Pandoc not available. Word conversion disabled.")
            except:
                st.warning("âš ï¸ Pandoc not available. Word conversion disabled.")
